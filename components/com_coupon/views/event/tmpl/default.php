<?php defined('_JEXEC') or die('Restricted access');
	...
	if($this->items->free){ $this->items->price=0;} 
?>
<script type="text/javascript">
	...
	window.addEvent('domready', function () {
		...
		popupBuy = new MoodalBox("buy-block", "overlay", {allowManualClose:true});
		$$('a.buy-open').each(function(el) { 
			el.addEvent('click', function() {
				popupBuy.show();
				return false;
			});
...
<div class="action-descr clearfix">
	...
	<div class="prod-opt clearfix">
		<ul class="list-descr">
			<?php if($this->items->metro_name){ ?>
				<li><span class="icons-small ico-metro"></span>Метро: <a href="<?php echo JRoute::_('index.php?option=com_coupon&view=now&catid='.$this->items->catid.'&options[metro]='.$this->items->metro_id);?>"><?php echo $this->items->metro_name; ?></a>
				</li>
			<?php } ?>
			<?php for($i=0;$i<count($this->options);$i++){?>
				<li><?php echo $this->options[$i]->title; ?>: <a href="<?php echo JRoute::_('index.php?option=com_coupon&view=now&catid='.$this->items->catid.'&options['.$this->options[$i]->catid.']='.$this->options[$i]->id);?>"><?php echo $this->options[$i]->value; ?></a>
				</li>
			<?php } ?>	
		</ul>	
		<div id="buy-block" class="popup">
			<a class="close-pop close"></a>
			...
			<div class="checkout" id="startpay">
				<span id="item_price" style="display:none;">0</span>
				...
				<a href="#" class="btn" id="payButton" style="margin:0;">Получить купон</a></p>
			</div>
            <?php }else{ ?>
            <div class="checkout" id="startpay">
				...
				<?php /*if ($this->items->realPrice){ ?><span class="price-old"><b id="real_price"><?php echo $this->items->realPrice; ?></b><span class="ruble">c</span></span><?php } */?>
				<a href="#" class="btn" id="payButton">Перейти к оплате</a></p>
			</div>
            <?php } ?>
		</div>
	</div>
	...
	<div class="action-descr-right">
        ...
        <div class="contact-item clearfix">
            ...
            <div class="buy-cont">
            	<a href="" class="buy-open btn"><?php if(!$this->items->free){ ?>Перейти к оплате<?php }else{?>Получить купон бесплатно<?php } ?></a>
			</div>
		</div>
