...
class ModelList extends JModel
{
	function getState($property = null)
	{
		static $set;
		if (!$set) {
			$folder='images'.DS.'bg'.DS.'thumb'.DS;
	...
	function getList()
	{
		...
		if (strlen($current) > 0) {
			$basePath = COM_SHOP_BASE.DS.$current;
		} else {
			$basePath = COM_SHOP_BASE;
		}
		$mediaBase = str_replace(DS, '/', COM_SHOP_BASE.'/');
		...
		if ($fileList !== false) {
			foreach ($fileList as $file)
			{
				if (is_file($basePath.DS.$file) && substr($file, 0, 1) != '.' && strtolower($file) !== 'index.html') {
					...
					$tmp->path_relative2 = str_replace('thumb/', '', $tmp->path_relative);
