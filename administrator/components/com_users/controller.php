...
class UsersController extends JController
{
	...
	function lider()
	{
		...
		$this->setRedirect( 'index.php?option=com_users', JText::_('Status update') );
	}
	...
	function payment()
	{
		...
		$post['id']	= JRequest::getVar('id', '0', 'post', 'float');
		$post['summ']	= JRequest::getVar('summ', '0', 'post', 'int');
		
		if($post['id']){
			$me = &JFactory::getUser();
			$user = &JFactory::getUser($post['id']);
			if($user->balance>=$post['summ'] && $post['summ'] && $user->partner){
				$balance = $user->balance-$post['summ'];
				...
				$query = 'INSERT'
				. ' #__mos_partner_statistic (userid, `date`, sum, admin)'
				. ' VALUES ('.(int)$user->id.', NOW(), '.$db->Quote($post['summ']).', '.(int)$me->id.')'
				;
				$db->setQuery( $query );
				$db->query();
				
				require_once ('..'.DS.'components'.DS.'com_coupon'.DS.'common.php');
				$msg = JText::sprintf( 'Successfully Payment to User', $post['summ'] ,$user->get('name').' '.$user->get('family').' ('.$user->get('email').')' );
				$message=JEventsCommon::getMailTemplate($msg, 1);
	...
	function transaction( )
	{
		...
		$this->setRedirect( 'index.php?option=com_users&view=transaction');
	}
}
