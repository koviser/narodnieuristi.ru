...
class JHTMLSelect
{
	...
	*/
	...
	function datelist($day, $month, $year){
			for($i=1;$i<32;$i++)
			{
				$items_day[$i] = JHTML::_('select.option',  $i, $i );
			}
			$month_title=array(' ', JText::_('January'), JText::_('February'), JText::_('March'), JText::_('April'), JText::_('May'), JText::_('June'), JText::_('July'), JText::_('August'), JText::_('September'), JText::_('October'), JText::_('November'), JText::_('December'));
			for($j=1;$j<13;$j++)
			{
				$items_month[$j] = JHTML::_('select.option',  $j, $month_title[$j] );
			}
			for($k=date("Y");$k>1930;$k--)
			{
				$items_year[$k] = JHTML::_('select.option',  $k, $k);
			}
		$html = '<div id="selectDay">'.JHTML::_('select.genericlist',   $items_day, 'day', 'class="inputbox" size="1" ', 'value', 'text', "$day" ).'</div>';	
		$html .= ' <div id="selectMonth">'.JHTML::_('select.genericlist',   $items_month, 'month', 'class="inputbox" size="1" ', 'value', 'text', "$month" ).'</div>';
		$html .= ' <div id="selectYear">'.JHTML::_('select.genericlist',   $items_year, 'year', 'class="inputbox" size="1" ', 'value', 'text', "$year" ).'</div>';	
		return $html;
	}
